angular.module("umbraco").controller("Gmaps.Core.Controller",
    [
        "$scope",
        "$http",
        "$element",
        ...
        function ($scope, $http, $element, GmapsCoreFactory) {
            'use strict';

            // Guru Digital
            $scope.defaultLocation = '-37.135251, 175.539330';
            // default zoomlevel
            $scope.zoomLevel = 5;
            ...
            var autoCompleteElement = $element.find("#map-autocomplete")[0];
            ...
            $http.get("../umbraco/GDMGoogleMaps/GoogleMapsApi/GetAPIKey").then(function (response) {
                if (response.data !== '') {
                    GmapsCoreFactory.mapsInitialized(response.data).then(function () {
                        // resolved
